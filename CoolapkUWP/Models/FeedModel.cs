...
namespace CoolapkUWP.Models
{
        ...
        public FeedModel(JObject token, FeedDisplayMode mode = FeedDisplayMode.normal) : base(token)
        {
            ...
            if (ShowRelationRows)
            {
                ...
                if (valuettitle != null && !string.IsNullOrEmpty(valuettitle.ToString()))
                    vs.Add(new RelationRowsItem { Title = valuettitle.ToString(), Url = token.Value<string>("turl"), Logo = new ImageModel(token.Value<string>("tpic"), ImageType.Icon) });
    ...
    internal class RelationRowsItem
    {
