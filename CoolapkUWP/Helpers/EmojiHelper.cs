...
"[SegoeUI滑稽]" };
        ...
        static string GetEmojiID(string key)
        {
            switch (key)
            {
        ...
        static string GetOldEmojiId(string key)
        {
            switch (key)
            {
        ...
        {
            string id = useOldEmoji ? GetOldEmojiId(key) : GetEmojiID(key);
            if (string.IsNullOrEmpty(id))
                id = SettingsHelper.Get<bool>("IsDarkMode") ? "img_placeholder_night" : "img_placeholder";
            else id = $"Emoji/{id}";
            return new Uri($"ms-appx:///Assets/{id}.png");
        }
    }
}
