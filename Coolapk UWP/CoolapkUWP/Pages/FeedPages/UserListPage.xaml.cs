...
namespace CoolapkUWP.Pages.FeedPages
{
    ...
    public sealed partial class UserListPage : Page
    {
        ...
        private async void LoadList(int p = -1)
        {
            ...
            JsonArray array = UIHelper.GetDataArray(await UIHelper.GetJson($"/user/{(isFollowList ? "followList" : "fansList")}?uid={uid}&page={(p == -1 ? ++page : p)}{(firstItem == 0 ? string.Empty : $"&firstItem={firstItem}")}{(lastItem == 0 ? string.Empty : $"&lastItem={lastItem}")}"));
